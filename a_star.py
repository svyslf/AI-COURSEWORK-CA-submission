...
def h_score(end:tuple, current:tuple):
    """
...
def a_star(representation:list[list[str]], start:tuple, end:tuple):
    """
    ...
    while not open_list.empty():
        current_position = open_list.get()[1]

        if current_position == end:
            break

        neighbours = adjacents(representation, current_position[0], current_position[1])
    
        for neighbour in neighbours:
            g_score_new = g_score[current_position] + 1
            ...
            if g_score_new < g_score[neighbour]:
                ...
                if neighbour not in open_list.queue:
                    open_list.put((f_score[neighbour], neighbour))
    try:
        fwd_path = {}
        goal = end
